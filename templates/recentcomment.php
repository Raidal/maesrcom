...
$sql = "SELECT DISTINCT ID, post_title, post_password, comment_ID, comment_post_ID, 
			...
			SUBSTRING(comment_content,1,25) AS com_excerpt FROM $wpdb->comments LEFT OUTER JOIN $wpdb->posts ON ($wpdb->comments.comment_post_ID = $wpdb->posts.ID) WHERE comment_approved = '1' 
			AND comment_type = '' 
