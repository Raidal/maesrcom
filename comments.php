...
?>
		...
		<?php if($comments) : ?><!-- 如果有评论 -->
			...
			<div class="commentshow">
				...
				<?php wp_list_comments('type=comment&callback=corparolecomment&max_depth=10000'); ?>
				</ol>
			...
			<ol class="pingtlist">
					<?php wp_list_comments('type=pings&callback=corparolepings'); ?>
			</ol>
			<?php else : ?>
			  <ol class="pingtlist">
			  <li>
		...
		<div id="respond">
		    ...
		    <?php if(get_option('comment_registration') && !$user_ID) : ?>
		        ...
		        <form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform" name="comm_frm">
		            ...
		            <?php if($user_ID) : ?>
		                <div class="writerinfodiv"><a href="<?php echo get_option('siteurl'); ?>/wp-admin/profile.php"><?php echo $user_identity; ?></a>已登录！ <a href="<?php echo get_option('siteurl'); ?>/wp-login.php?action=logout" title="Log out of this account">退出</a></div>
		    <?php else : ?>
			...
			<?php endif; ?>
		            ...
		            <p>
		            	<input name="submit" type="submit" id="submit" class="submitstyle normal" tabindex="5" value="提交评论(Ctrl+Enter)"/>
